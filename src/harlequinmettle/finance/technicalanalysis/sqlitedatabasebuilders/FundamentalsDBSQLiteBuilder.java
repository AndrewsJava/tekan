...
public class FundamentalsDBSQLiteBuilder {
	...
	private void buildDB() {
		...
		if (cn != null) {
			...
			int NUMBER_ENTRIES = CurrentFundamentalsSQLiteDatabase.forDisplaying.length + 2;
	...
	private ArrayList<String> makeListOfSomeData(String entry) {
		...
		List<String> labels = Arrays
				.asList(CurrentFundamentalsSQLiteDatabase.labels);
		String[] tickerDataPair = entry.split("\\^");
		String ticker = tickerDataPair[0];
		data.add(ticker);
		float[] rawData = DataUtil.validSmallDataSet(tickerDataPair[1], null);
		...
		for (String s : CurrentFundamentalsSQLiteDatabase.forDisplaying) {
